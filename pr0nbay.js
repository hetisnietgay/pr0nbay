...
let HiddenTags = GM_getValue(
    ...
    */
    ...
    // Note: the below base64 is the banner for the console banner followed by the actual HTML for the (wip) gui itself.
    // feel free to verify there's nothing spooky. The payload markers are for a build script that uses them as markers to locate where to write the new form data.
    ...
    */
    ...
    /*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/
    if (Thumbnailer) {
        $(".torrent_table > tbody > tr:first > td:first").after("<td>Screenshot</td>");
        torrents.each(function () {
            let str = $(this).find("script").html();
            let m;
            let src = "";
